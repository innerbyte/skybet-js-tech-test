...
class FootballLeague extends Component {
    ...
    init() {
        let cat_name = this.props.events.find((v, i) => {
			return v.cat_name === this.props.match.params.cat_name;
		}).type_name;
	...
	on_event_click(event) {
		this.props.history.push(`/football/${this.props.match.params.cat_name}/${event.eventId}`);
	}
	...
	event_item(event, key, is_odd) {
		...
		return (
			<Col key={key} xs="12" sm="12" md="12" style={{cursor: 'pointer', backgroundColor: is_odd ? '#dddddd' : '#ffffff'}} onClick={ (e) => { this.on_event_click(event); } } >
